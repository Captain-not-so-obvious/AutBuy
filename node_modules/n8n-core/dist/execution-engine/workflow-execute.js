...
class WorkflowExecute {
    ...
    async runNode(workflow, executionData, runExecutionData, runIndex, additionalData, mode, abortSignal) {
        ...
        if (nodeType.execute || customOperation) {
            ...
            const jsonCompatibleResult = (0, is_json_compatible_1.isJsonCompatible)(data);
            if (!jsonCompatibleResult.isValid) {
                di_1.Container.get(error_reporter_1.ErrorReporter).error(new n8n_workflow_1.UnexpectedError('node execution output incorrect data'), {
                    extra: {
                        nodeName: node.name,
                        nodeType: node.type,
